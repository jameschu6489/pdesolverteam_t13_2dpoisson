...
def train():
    ...
    def get_loss_and_flat_grad(xy, xy_b):
        def loss_and_flat_grad(weights):
            set_weights(Get_Loss, weights, indexs)
            loss_value = train_net_lbfgs(xy, xy_b)
            grads = train_net_lbfgs.grad(Get_Loss, train_net_lbfgs.weights)(xy, xy_b)
    ...
    loss_and_flat_grad = get_loss_and_flat_grad(xy, xy_b)
